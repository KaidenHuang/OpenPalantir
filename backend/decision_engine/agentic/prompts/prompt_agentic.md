...
{{
  "summary": "决策摘要（一句话）",
  "situation_analysis": "现状分析（详细描述当前情况）",
  "key_issues": [
  ...
  "recommendation": {{"option": "推荐方案名", "reason": "推荐理由"}},
  "work_orders": [
...
9. **必须生成 work_orders**：只要问题涉及行动方案/改进建议/问题处理，必须生成至少 1 个行动工单（含具体步骤和验收标准）。简单问候或纯信息查询可以为空
