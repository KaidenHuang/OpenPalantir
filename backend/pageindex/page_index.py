...
################### meta_processor #########################################################
async def meta_processor(page_list, mode='process_no_toc', start_index=1, toc_content=None, toc_page_list=None, opt=None):
    if mode == 'process_no_toc':
        print('process_no_toc')
        return process_no_toc(page_list, start_index=start_index, model=opt.model)
    elif mode == 'process_toc_with_page_numbers':
        print('process_toc_with_page_numbers')
        return process_toc_with_page_numbers(toc_content, toc_page_list, page_list, opt.toc_check_page_num, model=opt.model)
    elif mode == 'process_toc_no_page_numbers':
        print('process_toc_no_page_numbers')
        return process_toc_no_page_numbers(toc_content, toc_page_list, page_list, start_index=start_index, model=opt.model)
    else:
...
async def page_index_builder(doc, page_list, opt):
    """PageIndex 树结构构建器（异步，用于 async/await 上下文）。"""
